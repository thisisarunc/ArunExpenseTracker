...
const UPI_PATTERN = /(?:upi[:\-\/]|vpa[:\-])\s*([a-zA-Z0-9._-]+@[a-zA-Z]+)/i;
const CARD_PATTERN = /(?:card|ac|a\/c|acct)[\s.]*(?:no\.?|ending|xx+)?\s*[xX*]*(\d{4})/i;
const MERCHANT_PATTERNS = [
  /(?:at|to|from|@|paid to|received from|trf to|trf from)\s+([A-Za-z0-9\s&.'_-]{2,30}?)(?:\s+on|\s+ref|\s+upi|\.|,|$)/i,
...
function guessCategory(message, merchant = '') {
  const text = (message + ' ' + (merchant || '')).toUpperCase();
...
export function isBankSender(sender) {
  ...
  const upperSender = sender.toUpperCase().replace(/[^A-Z0-9]/g, '');
